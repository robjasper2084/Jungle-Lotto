...
The Pages artifact contains 1,589 files and measures 944.2 MiB under the 1,200 MiB gate. The packager omitted 134 unreferenced media files totaling 400.1 MiB without modifying the source media. The News static build caches publisher-supplied article imagery locally.
...
Use the reviewed pull request from `upgrade-redesign` to `main` and select **Create a merge commit**. Do not squash or rebase. The owner supplied the exact authorization phrase `APPROVE PRODUCTION MERGE` after the complete `v2-rc8` gate passed; merge only the immutable tagged candidate and verify the resulting GitHub Pages deployment.
