...
const s=new RideSimulation() as RideSimulation & {obstacles: object[]; speed:number; travel:number; x:number; health:number};s.obstacles=[];s.start();for(let i=0;i<120;i++)s.step(CONFIG.step,{});assert.ok(Math.abs(s.speed-3)<1e-6);assert.ok(s.travel>1.49&&s.travel<1.53);
const braking=new RideSimulation() as typeof s;braking.obstacles=[];braking.start();braking.speed=8;for(let i=0;i<120;i++)braking.step(CONFIG.step,{brake:true});assert.ok(Math.abs(braking.speed-1)<1e-6,'Braking removes 7 m/s in one second');for(let i=0;i<30;i++)braking.step(CONFIG.step,{brake:true});assert.equal(braking.speed,0,'Brake stops without reversing');
const steering=new RideSimulation() as typeof s;steering.obstacles=[];steering.start();for(let i=0;i<120;i++)steering.step(CONFIG.step,{steer:1});assert.equal(steering.x,CONFIG.width/2-CONFIG.radius,'Steering respects path edge');
