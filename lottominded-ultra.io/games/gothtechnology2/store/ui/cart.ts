...
export function initCart(products: Product[]) {
  const provider: CommerceProvider = createProvider(storage(), products);
  ...
  async function ensureCart(): Promise<Cart> {
    ...
    return initialization ??= (async () => {
      const id = mode === 'commerce' ? saved(key) : 'demo-loadout';
  ...
  async function checkout() {
    ...
    try {
      ...
      if(current.demo || conversionMode(products) !== 'commerce') throw new Error('Store preview: the owner must approve product details, policies, and checkout before payments open.');
  ...
  document.addEventListener('click', event=>{
    ...
    if(line && target.closest('[data-remove-line]')) { analytics.trackEvent('remove_from_loadout',{handle:line.productHandle,quantity:line.quantity}); void mutate(c=>provider.removeCartLine(c.id,line.id),'Item removed from your loadout.'); }
    ...
    if(quick) showQuick(quick.dataset.quickView!,quick);
    const direct=target.closest<HTMLButtonElement>('[data-save-product]');
  ...
  function updateForm(form: HTMLFormElement) {
    ...
    const notice=$('[data-variant-status]',form); if(notice)notice.textContent=!variant?mode==='interest'?'Choose a size and color to save this concept.':'Choose a size and color.':!variant.available?'This variant is unavailable.':mode==='interest'?'Concept options — final sizes and availability are pending.':'Available. Final inventory and price checked at checkout.';
  ...
  function showQuick(handle:string, trigger:HTMLElement) {
