...
export function initCatalog(products:Product[]) {
  ...
  if(form&&grid) {
    ...
    const update=(write=true)=>{
      ...
      if(write){const url=new URL(location.href);fields.forEach(name=>filters[name]!==defaults[name]?url.searchParams.set(name,filters[name]??''):url.searchParams.delete(name));history.replaceState(null,'',url);}
    };
