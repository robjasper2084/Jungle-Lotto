...
export const GET:APIRoute=async()=> {
  const paths=['','shop/','collections/','lookbook/','play/','lottomind/','about/',...Object.keys(supportPages).map(p=>p+'/'),...(await buildProvider.getProducts()).map(p=>'products/'+p.handle+'/'),...(await buildProvider.getCollections()).map(c=>'collections/'+c.handle+'/')];
