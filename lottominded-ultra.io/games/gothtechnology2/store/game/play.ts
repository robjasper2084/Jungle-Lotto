...
import {rewardGameForURL,rewardGameForNavigation} from '../public/arcade/games.js';
export function initPlay() {
  ...
  function syncGame(navigatingTo?:string){
    try{
      const url=navigatingTo||frame?.contentWindow?.location.href;
      ...
      if(navigatingTo){ready=false;notice.textContent='Loading '+game.title+'…';return;}
  ...
  window.addEventListener('message',event=>{
    ...
    const navigation=rewardGameForNavigation(event,frame?.contentWindow??null,storeBase);
    if(navigation){syncGame(new URL(navigation.path,storeBase).href);return;}
