...
export function initPlay() {
  ...
  let frame:HTMLIFrameElement|null=null,timer:ReturnType<typeof setTimeout>|undefined;
  ...
  launch.addEventListener('click',()=>{
    ...
    frame.addEventListener('load',()=>{notice.textContent='Game loaded. Preparing the character systems…';});
  ...
  window.addEventListener('message',event=>{
    ...
    if(data.type==='GOTHTECH_GAME_READY'){clearTimeout(timer);notice.textContent='Game ready. Use the in-game controls or keyboard. Back to Store remains above the game.';}
