...
function nodes(connection: any): any[] { return connection?.nodes ?? connection?.edges?.map((edge: any) => edge.node) ?? []; }
function safeImage(url: string): string {
  try { const parsed = new URL(url); return parsed.protocol === 'https:' ? parsed.href : ''; } catch { return ''; }
}
function variant(raw: any): Variant {
  const option = (name: string) => raw.selectedOptions?.find((o: any) => o.name.toLowerCase() === name)?.value;
...
export function normalizeProduct(raw: any): Product {
  ...
  return { id: raw.id, handle: raw.handle, title: raw.title, subtitle: raw.productType || 'GOTHTECHNOLOGY equipment',
    ...
    tags: raw.tags ?? [], images: nodes(raw.images).map((im, i) => ({ src: safeImage(im.url), alt: im.altText || raw.title,
      label: i === 0 ? 'Front' : `View ${i + 1}`, width: im.width, height: im.height })).filter(im => im.src),
    model: null, video: null, colors: [...new Set(variants.map(v => v.color))], sizes: [...new Set(variants.map(v => v.size))], variants,
    inventory: null, materials: 'See the product description for confirmed materials.', fabricWeight: 'See the product description.',
    finish: 'See the product description for production details.', careInstructions: 'Follow the care label supplied with your item.',
    shippingMessage: 'Delivery options and charges are calculated in Shopify checkout.', returnMessage: 'Review the owner-approved return policy before ordering.',
...
export function normalizeCart(raw: any): Cart {
  ...
  const lines = nodes(raw.lines).map(line => {
    ...
    return { id: line.id, variantId: v.id, productHandle: merchandise.product.handle, title: merchandise.product.title,
      ...
      size: v.size, color: v.color, quantity: line.quantity,
...
export class ShopifyProvider implements CommerceProvider {
  ...
  private async request(query: string, variables: Record<string, unknown> = {}): Promise<any> {
    ...
    if (result.errors?.length || !result.data) throw new Error('Shopify could not complete this request. Check Storefront API access and configuration.');
    return result.data;
  }
  private async completeVariants(raw: any) {
    while (raw.variants.pageInfo?.hasNextPage) {
      const cursor = raw.variants.pageInfo.endCursor;
      const data = await this.request(`query Variants($handle: String!, $after: String) { product(handle: $handle) { variants(first:250, after:$after) { nodes { ${variantFields} } pageInfo { hasNextPage endCursor } } } }`, { handle: raw.handle, after: cursor });
  ...
  async getProducts(): Promise<Product[]> {
    ...
    do {
      const data = await this.request(`query Products($after:String) { products(first:50, after:$after, sortKey:CREATED_AT) { nodes { ${productFields} } pageInfo { hasNextPage endCursor } } }`, { after });
      for (const raw of nodes(data.products)) products.push(await this.completeVariants(raw));
      const next = data.products.pageInfo.hasNextPage ? data.products.pageInfo.endCursor : null;
  ...
  async getProduct(handle: string) {
    const data = await this.request(`query Product($handle:String!) { product(handle:$handle) { ${productFields} } }`, { handle });
    return data.product ? this.completeVariants(data.product) : null;
  }
  async getCollections(): Promise<Collection[]> {
    const all: Collection[] = []; let after: string | null = null;
    do {
      const data = await this.request('query Collections($after:String) { collections(first:100, after:$after) { nodes { handle title description } pageInfo { hasNextPage endCursor } } }', { after });
      all.push(...nodes(data.collections));
      const next = data.collections.pageInfo.hasNextPage ? data.collections.pageInfo.endCursor : null;
  ...
  private async mutate(name: string, declarations: string, args: string, variables: Record<string, unknown>): Promise<Cart> {
    const data = await this.request(`mutation Change(${declarations}) { ${name}(${args}) { cart { ${cartFields} } userErrors { message } warnings { message } } }`, variables);
    ...
    cart.warnings = (payload.warnings ?? []).map((w: any) => w.message);
    return cart;
  }
  async createCart() { return this.mutate('cartCreate', '$input:CartInput!', 'input:$input', { input: {} }); }
  async getCart(id: string) {
    const data = await this.request(`query Cart($id:ID!) { cart(id:$id) { ${cartFields} } }`, { id });
