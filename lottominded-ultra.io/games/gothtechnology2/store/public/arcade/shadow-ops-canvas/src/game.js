(() => {
  ...
  const PLAYER_ROWS = globalThis.RahbeCore.PLAYER_ROWS;
  ...
  let run = null;
  let arcadeAudioVolume = 1, arcadeCutscenePaused = false;
  ...
  const MOVEMENT = globalThis.RahbeCore.MOVEMENT;
  ...
  function beginCutscene(runMode = pendingRunMode) {
    ...
    video.pause();
    video.currentTime = 0;
    video.muted = QUERY.get('arcade') === '1' && !(settings.sound || settings.music);
    video.volume = arcadeAudioVolume;
    setMode("cutscene");
  ...
  function startRun(runMode = pendingRunMode) {
    window.GothGameRewardFlush?.(arcadeSnapshot());
  ...
  function updatePlayer(state, dt, p) {
    ...
    if (playerActionPressed(p, "dash") && p.dashCd <= 0 && (p.grounded || !p.airDashUsed)) {
      ...
      p.h = p.standingH;
      p.y = oldBottom - p.h;
      globalThis.RahbeCore.beginDash(p, MOVEMENT);
  ...
  function performBufferedJump(state, p) {
    ...
    if (canWallJump && !(p.grounded || p.coyote > 0)) {
      ...
      globalThis.RahbeCore.launchJump(p, MOVEMENT.jumpVelocity);
  ...
  function firePlayerShot(state, p) {
    ...
    const spec = globalThis.RahbeCore.weaponSpec(weapon, p.overdriveTime > 0);
  ...
  function angledShot(shot, radians) { return globalThis.RahbeCore.angledShot(shot, radians); }
  ...
  function activateOverdrive(state, p) {
    globalThis.RahbeCore.beginOverdrive(p, 4.2);
  ...
  function collectPickup(state, pickup, p = state.player) {
    ...
    if (pickup.type === "shard") {
      state.shards += 1;
      state.arcadeShards = (state.arcadeShards || 0) + 1;
      addScore(state, 45);
  ...
  function finishRun(victory) {
    if (!run) return;
    run.arcadeVictory = Boolean(victory);
  ...
  function syncGameMusic() {
    ...
    const track = ensureGameMusic();
    track.volume = currentMusicVolume() * arcadeAudioVolume;
  ...
  function playTone(freq, duration, type = "sine", gainValue = 0.04) {
    if (!settings.sound || !audioCtx || arcadeAudioVolume === 0) return;
    setupAudioGraph();
    const now = audioCtx.currentTime;
    const destination = sfxMaster || audioCtx.destination;
    const peak = Math.min(0.13, Math.max(0.01, gainValue * SFX_GAIN_BOOST * arcadeAudioVolume));
    globalThis.RahbeCore.tone(audioCtx, freq, duration, type, peak, destination);
  }
  ...
  function arcadeSnapshot() {
    if (!run) return {runId:'',mode,seconds:0};
    const completed = run.levelResults?.length || 0;
    const bosses = completed + (run.bossDefeated && !run.levelResults.some(level => level.id === run.level.id) ? 1 : 0);
    return {runId:run.arcadeRunId,mode,seconds:Math.floor(run.campaignTime),score:run.stats.score,kills:run.stats.kills,shards:run.arcadeShards||0,bosses:Math.min(3,bosses),depth:0,seals:0,completions:run.arcadeVictory?1:0};
  }
  window.RahbeArcadeGame = {
    get ready() {return QUERY.get('arcade')!=='1' || [...loadedAssetKeys].every(key => images[key]?.complete && images[key]?.naturalWidth);},
    get progress() {const keys=[...loadedAssetKeys];return keys.length?keys.filter(key=>images[key]?.complete&&images[key]?.naturalWidth).length/keys.length:0;},
    getStats:arcadeSnapshot,
    pause:()=>{if(mode==='cutscene'){arcadeCutscenePaused=true;dom.introCutscene?.pause();}else pauseRun();},
    resume:()=>{if(arcadeCutscenePaused){arcadeCutscenePaused=false;dom.introCutscene?.play().catch(()=>{});}else if(mode==='paused')resumeRun();canvas.focus();},
    start:()=>startRun(),
    applySettings:values=>{settings.sound=!!values.sound;settings.music=!!values.music;settings.reducedMotion=window.matchMedia('(prefers-reduced-motion: reduce)').matches||!!values.reducedMotion||values.particles===false;settings.reducedShake=!!values.reducedShake;settings.highContrastShots=!!values.highContrast;arcadeAudioVolume=Math.min(1,Math.max(0,values.volume??.65));if(dom.introCutscene){dom.introCutscene.volume=arcadeAudioVolume;dom.introCutscene.muted=!(settings.sound||settings.music);}applySettings();syncGameMusic();},
    destroy:()=>{clearInputState();gameMusic?.pause();dom.introCutscene?.pause();audioCtx?.suspend().catch(()=>{});}
  };
})();
