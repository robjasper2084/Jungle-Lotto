...
export async function mountModel(host:HTMLElement,buttons:HTMLElement,path:string,product:Product,fallback:()=>void) {
  ...
  try {
    const gltf=await new GLTFLoader().setMeshoptDecoder(MeshoptDecoder).loadAsync(url.href);object=gltf.scene;scene.add(object);
    ...
    const status=host.parentElement?.querySelector('[data-viewer-status]');if(status)status.textContent='3D model loaded. Drag to rotate, use the view controls, or press the arrow keys.';
