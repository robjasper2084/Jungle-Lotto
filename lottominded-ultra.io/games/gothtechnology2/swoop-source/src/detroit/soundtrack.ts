...
export class Soundtrack {
 readonly audio=new Audio();private effect=new Audio();enabled=true;volume=.38;scene:MusicScene='ride';station='auto';private unlocked=false;private index=0;private now:HTMLElement;private toggle:HTMLButtonElement;private selector:HTMLSelectElement;private sequence:Record<string,number>={};private playing=false;private blocked=false;private gain=0;private ended=false;private persistStatus:HTMLElement;
 constructor(parent:HTMLElement){
  try{const p=JSON.parse(localStorage.getItem('swoop-music-v1')??'{}');this.enabled=p.enabled!==false;if(Number.isFinite(p.volume))this.volume=Math.max(0,Math.min(1,p.volume));if(['auto','all','bonus',...TRACKS.map(t=>t.id)].includes(p.station))this.station=p.station;}catch{}
  ...
  for(const[value,label]of [['auto','Auto · match my ride'],['all','All music'],['bonus','Bonus · LottoMind radio'],...TRACKS.filter(t=>t.group!=='effect'&&!('duplicateOf'in t)).map(t=>[t.id,t.title])]){const option=document.createElement('option');option.value=value;option.textContent=label;this.selector.append(option);}
 ...
 private save(){try{localStorage.setItem('swoop-music-v1',JSON.stringify({enabled:this.enabled,volume:this.volume,station:this.station}));}catch{this.persistStatus.textContent='Settings work for this session; browser storage is unavailable.';}}
 private list(){return TRACKS.filter(t=>!('duplicateOf'in t)&&t.group!=='effect'&&(this.station==='all'?t.group!=='bonus':this.station==='bonus'?t.group==='bonus':this.station==='auto'?t.group===this.scene:t.id===this.station));}
 unlock(){this.unlocked=true;this.blocked=false;if(!this.enabled)return;if(!this.audio.src)this.next();else this.play();}
 private play(){if(!this.unlocked||!this.enabled||document.hidden||this.blocked)return;this.playing=true;void this.audio.play().then(()=>{this.label();this.now.textContent=this.list().find(t=>this.audio.src.endsWith(t.url))?.title??'Digital Static radio';}).catch(()=>{this.playing=false;this.blocked=true;this.now.textContent='Tap to enable music playback.';this.toggle.textContent='Tap to play music';});}
 next(){const tracks=this.list();if(!tracks.length)return;const key=this.station==='auto'?this.scene:this.station;this.index=(this.sequence[key]??0)%tracks.length;this.sequence[key]=this.index+1;const track=tracks[this.index];this.audio.pause();this.blocked=false;this.audio.src=track.url;this.audio.volume=0;this.gain=0;this.now.textContent=track.title;this.ended=false;this.play();}
