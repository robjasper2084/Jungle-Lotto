...
The game uses Phaser 3 for the irregular board, camera, environment, and token animation; TypeScript modules own all deterministic rules; semantic DOM owns setup, HUD, dialogs, settings, and results.

## Boundaries

- `app/src/engine`: serializable state, actions, reducer, RNG, movement, economy, signals, scoring, and CPU policy. No Phaser imports.
- `app/src/content`: 36-node graph, eight networks, 24 ventures, and 48 cards.
