...
export function reducer(state:GameState, action:GameAction):GameState {
  ...
  switch(action.type){
    ...
    case "SIGNAL_REORDER": {if(player.lockedSignals.includes(action.from)||player.lockedSignals.includes(action.to))return state;player.signals=reorderSignal(player.signals,action.from,action.to);log(next,action,`${player.name} reordered the Signal Strip.`);return next;}
    case "SIGNAL_LOCK": {if(action.index<0||action.index>=player.signals.length)return state;player.lockedSignals=player.lockedSignals.includes(action.index)?player.lockedSignals.filter((index)=>index!==action.index):[...player.lockedSignals,action.index];log(next,action,`${player.name} ${player.lockedSignals.includes(action.index)?"locked":"unlocked"} Signal digit ${action.index+1}.`);return next;}
    case "SIGNAL_REPLACE": {if(player.focus<1||action.digit<0||action.digit>9||player.lockedSignals.includes(action.index))return state;player.signals=replaceSignal(player.signals,action.index,action.digit);player.focus--;log(next,action,`${player.name} replaced a Signal digit using Focus.`);return next;}
    ...
    case "MANUAL_SAVE": {log(next,action,`${player.name} saved the match manually.`);return next;}
