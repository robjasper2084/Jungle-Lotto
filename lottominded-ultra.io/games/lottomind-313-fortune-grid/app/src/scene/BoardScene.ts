...
export class BoardScene extends Phaser.Scene {
  ...
  create() {
    this.game.canvas.setAttribute("aria-label", "Illustrated Detroit Fortune Grid map");
  ...
  private drawEnvironment() {
    ...
    const circuitTitle = this.add
      .text(700, 388, "DETROIT VENTURE CIRCUIT", {
        fontFamily: "Arial",
        fontSize: "28px",
        fontStyle: "bold",
        color: "#fff1b5",
      ...
      .setOrigin(0.5)
      .setDepth(-6);
    const circuitNote = this.add
      .text(700, 430, "CLOCKWISE  •  PASS 313 HUB: +200 DD", {
        fontFamily: "Arial",
        fontSize: "14px",
        color: "#8beeff",
        backgroundColor: "rgba(5, 3, 10, .72)",
        padding: { x: 10, y: 6 },
      })
      .setOrigin(0.5)
      .setDepth(-6);
    this.board.add([circuitTitle, circuitNote]);
  }
  ...
  private drawBoard() {
    ...
    boardNodes.forEach((node) => node.edges.forEach((edge) => {
    ...
    route.lineStyle(24, 0x07050d, 0.82);
    segments.forEach(([ax, ay, bx, by]) => route.lineBetween(ax, ay, bx, by));
    route.lineStyle(10, 0xf1c75b, 0.72);
    segments.forEach(([ax, ay, bx, by]) => route.lineBetween(ax, ay, bx, by));
    route.lineStyle(3, 0x61e7ff, 0.72);
    segments.forEach(([ax, ay, bx, by]) => route.lineBetween(ax, ay, bx, by));
    ...
    boardNodes.forEach((node) => {
      ...
      const space = node.kind === "venture"
        ? this.add.rectangle(0, 0, 38, 46, 0x110b20, 0.98).setStrokeStyle(3, districtColors[Math.floor(node.ventureId! / 3)])
        : this.add.circle(0, 0, node.kind === "hub" ? 29 : 23, 0xb78432, 0.98).setStrokeStyle(3, 0xffe49a);
      ...
      if (node.kind === "venture") {
        const marker = this.add
          .text(0, 9, districts[Math.floor(node.ventureId! / 3)].symbol, {
            fontFamily: "Arial",
            fontSize: "15px",
            color: "#ffffff",
          })
          .setOrigin(0.5);
        const building = this.add.image(0, -19, "venture-level-1").setDisplaySize(56, 56).setVisible(false);
