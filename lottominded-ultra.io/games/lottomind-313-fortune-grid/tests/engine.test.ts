...
describe("deterministic rules",()=>{
  ...
  it("locks Signal digits against replacement and records manual saves",()=>{let s=createInitialState({mode:"quick313",playerCount:2,localPlayers:1,seed:313});s.players[0].signals=[3,1,3];s=reducer(s,{type:"SIGNAL_LOCK",index:0});expect(s.players[0].lockedSignals).toEqual([0]);const unchanged=reducer(s,{type:"SIGNAL_REPLACE",index:0,digit:9});expect(unchanged.players[0].signals).toEqual([3,1,3]);s=reducer(s,{type:"MANUAL_SAVE"});expect(s.eventLog.at(-1)?.message).toContain("saved the match")});
