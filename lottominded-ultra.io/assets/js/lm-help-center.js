(() => {
  ...
  if (!search || !filters || !articles.length || !status) return;
  ...
  const update = () => {
    ...
    articles.forEach((article) => {
      const text = [article.querySelector("summary")?.textContent, article.dataset.helpKeywords, article.dataset.helpCategory, article.textContent].join(" ").toLowerCase();
    ...
    status.textContent = visible ? `${visible} help ${visible === 1 ? "topic" : "topics"} shown.` : "No help topics match. Try a broader search or contact support.";
  };
